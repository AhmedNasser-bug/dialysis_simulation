...
def generate_absolute_worst_burst(config: SimulationConfig, seed: int = 42) -> ShiftScenario:
    patient_count = config.patient_volume.high
    nurse_count = config.nurse_count.low
    total_machines = config.total_machines.low
...
def generate_absolute_worst_uniform(config: SimulationConfig, seed: int = 42) -> ShiftScenario:
    patient_count = config.patient_volume.high
    nurse_count = config.nurse_count.low
    total_machines = config.total_machines.low
    ...
    for pid in range(1, patient_count + 1):
        patient_arrivals.append({
            "id": pid,
            "arrival_min": config.sample_arrival_minute(rng), # Uniform arrivals
            "setup_min": max_setup,
            "session_min": config.sample_session_duration_minutes(rng)
        })
...
def get_all_edge_cases(config: SimulationConfig) -> Dict[str, ShiftScenario]:
    return {
        ...
        "Absolute Worst (Burst)": generate_absolute_worst_burst(config),
        "Absolute Worst (Uniform)": generate_absolute_worst_uniform(config)
    }
