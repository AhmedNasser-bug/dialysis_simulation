...
class PatientSession:
    ...
    session_start: int = 0
    session_end: int = 0
    actual_session_duration: float = 0.0   # actual minutes of dialysis received
    prescribed_session_duration: float = 0.0  # originally prescribed minutes
    wait_time: float = 0.0
...
def cap_session_to_shift(
    setup_end: int,
    prescribed_duration: int,
    scenario: ShiftScenario,
) -> tuple[float, bool]:
    """
    Apply the hard shift-wall cap to a patient's dialysis session.

    Returns
    -------
    actual_duration : float
        The actual minutes of dialysis the patient will receive (capped at shift wall).
    is_truncated : bool
        True if the session was cut short by the wall (but still viable).
    """
    available = max(0, scenario.shift_end_minutes - setup_end)
    actual = min(prescribed_duration, available)
    is_truncated = actual < prescribed_duration
    return float(actual), is_truncated
...
def aggregate_shift_statistics(
    ...
    served = [p for p in patients if not p.failed_to_serve]
    ...
    if served:
        avg_session = sum(p.actual_session_duration for p in served) / len(served)
    else:
        avg_session = 0.0

    sessions_truncated = sum(
        1 for p in served
        if p.actual_session_duration < p.prescribed_session_duration
    )
    ...
    return ShiftStatistics(
        ...
        avg_session_time_minutes=avg_session,
        sessions_truncated_count=sessions_truncated,
