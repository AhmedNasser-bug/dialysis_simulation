...
def create_dataframes(shift_statistics: List[ShiftStatistics]) -> Dict[str, pd.DataFrame]:
    ...
    raw_df = raw_df.astype({
        ...
        'avg_session_time_minutes': 'float64',
        'sessions_truncated_count': 'int32',
    })
    ...
    for strategy_name in raw_df['strategy_name'].unique():
        ...
        mean_session = strategy_data['avg_session_time_minutes'].mean()
        std_session = strategy_data['avg_session_time_minutes'].std(ddof=1)
        ci_95_session = 1.96 * std_session / np.sqrt(n_iterations) if n_iterations > 1 else 0.0
        total_truncated = strategy_data['sessions_truncated_count'].sum()
        mean_truncated = strategy_data['sessions_truncated_count'].mean()
        ...
        summary_data.append({
            ...
            'mean_avg_session_time_minutes': mean_session,
            'std_avg_session_time_minutes': std_session,
            'ci_95_avg_session_time_minutes': ci_95_session,
            'total_sessions_truncated': total_truncated,
            'mean_sessions_truncated_per_shift': mean_truncated,
        })
...
def calculate_paired_differences(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Calculate differences (alphabetically first vs others)
    ...
    for i in range(len(pivot_data[reference_strategy])):
        ...
        for strategy in strategy_list[1:]:
            ...
            row_diff[f'delta_avg_session_{strategy}_vs_{reference_strategy}'] = (
                comp_row['avg_session_time_minutes'] - ref_row['avg_session_time_minutes']
            )
...
def generate_statistical_report(dataframes: Dict[str, pd.DataFrame]) -> str:
    ...
    for _, row in summary_df.iterrows():
        report_lines.extend([
            ...
            f"    Avg Session Duration: {row['mean_avg_session_time_minutes']:.2f} +/- {row['ci_95_avg_session_time_minutes']:.2f} min (95% CI)",
            f"    Truncated Sessions (total): {row['total_sessions_truncated']}",
            f"    Truncated Sessions (mean/shift): {row['mean_sessions_truncated_per_shift']:.2f}",
            "",
